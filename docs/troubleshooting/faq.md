...
### How do I update DClaw Email?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp email --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Email without Kubernetes?

Yes, for local development:

```bash
cd dclaw-email/frontend && npm run dev
cd dclaw-email/backend && uvicorn dclaw_email.main:app --reload
```
