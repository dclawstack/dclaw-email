...
# Check events
kubectl get events -n dclaw-email --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-email deployment/dclaw-email-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-email-db-credentials -n dclaw-email
   ```
